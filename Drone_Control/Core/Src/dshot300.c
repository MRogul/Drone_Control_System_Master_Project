...
{
    for (int i = 0; i < 400; i++) // skrócone do ok. 2s
    {
        dshot_send_motor(motor_index, 0);
        HAL_TIM_PWM_Start_DMA(&htim1, TIM_CHANNEL_1 + (motor_index * 4),
                              (uint32_t*)dshot_dmabuffer_ccr[motor_index],
...
{
    for (int i = 0; i < 400; i++) // skrócone z 1000 -> ~2s
    {
...
{
    uint16_t value = (spin_dir == DSHOT_SPIN_DIRECTION_NORMAL) ? 20 : 21;
    for (int i = 0; i < 10; i++)
    {
